...
backend default {
	# Redirection vers le serveur PHP-FPM.
	# FONCTIONNE SEULEMENT EN PRODUCTION.
	.host = "php";
	.port = "9000";
}
...
sub vcl_recv {
	...
	if (req.http.Cookie) {
		...
		set req.http.Cookie = regsuball(req.http.Cookie, ";(PHPSESSID,__SECURE-PHPSESSID,SYMFONY_COOKIE)=", "; \1=");
		set req.http.Cookie = regsuball(req.http.Cookie, ";[^ ][^;]*", "");
		set req.http.Cookie = regsuball(req.http.Cookie, "^[; ]+|[; ]+$", "");
